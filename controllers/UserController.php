...
public function displayProfile(): void
{
   ...
   $avatarItems = $am->findAllAvatars();
    ...
    // Générer token CSRF
    $csrfToken = $tm->generateCSRFToken();
...
public function updateProfile(): void
{
    ...
    try {
        ...
        error_log("=== UPDATE PROFILE === Succès");
        ...
        error_log("=== UPDATE PROFILE === Erreur: " . $e->getMessage());
...
public function resetPasswordFromProfile(): void
{
    ...
    try {
        ...
        // Mettre à jour (password + statut = 0)
        ...
        // Déconnecter l'utilisateur (son statut est maintenant 0)
        $_SESSION['user']['statut'] = 0;
        
        echo json_encode([
            'success' => true,
            'message' => 'Mot de passe réinitialisé ! Un email vous a été envoyé. Vous allez être déconnecté.',
